...
# regulation previously found Mito1 and MB2. 
...
new.pc1 <- PC1VecFun(RNA.ma.pat2[-zero.rows,],new.sort,n = 10) *-1
...
new.MB2.sort.df <- data.frame(X = colnames(RNA.ma.pat)[new.sort],
                          MB2.pc1 = new.pc1, MB2.index =  seq_len(length(new.sort)),
                          PAM50 = (RNAseq.pam50$Call)[-s1][new.sort],
                          LumA = (RNAseq.pam50$LumA)[-s1][new.sort],
                          LumB = (RNAseq.pam50$LumB)[-s1][new.sort])


# WriteXLS(new.sort.df, 'TCGA_RNASeq_nonorm_MB2.xlsx')

TCGA.MB2.cv <- CVEval(RNA.ma.pat[-zero.rows,], log2(RNA_seq_v2_mat[,-s1] + 1),
                  new.sort[seq(10)],splits = 8)

TCGA.MB2.CV.RNAseq.df <- data.frame(Genes = rownames(RNA_seq_v2_mat),
                                CV = as.numeric(TCGA.MB2.cv))


TCGA.MB2.RNAseq.df <- new.MB2.sort.df 
save(list = c('TCGA.MB2.RNAseq.df','TCGA.MB2.CV.RNAseq.df'),
     file = 'TCGA_MB2_RNASeq_data_nonorm.RData')



# MB2 lookup ----
ggplot(new.MB2.sort.df , aes(MB2.index, MB2.pc1)) +
