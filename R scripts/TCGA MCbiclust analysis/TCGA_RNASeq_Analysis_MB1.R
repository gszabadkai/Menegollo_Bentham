...
# regulation previously found in microarray data. 
...
gene.group1 <- BRCA.genelist[gene.group1.loc]
gene.group2 <- BRCA.genelist[gene.group2.loc]
...
RNA.ma.pat2 <- RNA.ma.pat - rowMeans(RNA.ma.pat) # colMeans??
...
cv.test <- CVEval(RNA.ma.pat[-zero.rows,], log2(RNA_seq_v2_mat + 1),
...
ggplot(as.data.frame(new.pc1), aes(x=new.pc1)) + geom_density()
...
new.MB1.sort.df <- data.frame(X = colnames(RNA.ma.pat)[new.sort],
                          MB1.pc1 = new.pc1, MB1.index =  seq_len(length(new.sort)),
...
TCGA.MB1.cv <- CVEval(RNA.ma.pat[-zero.rows,], log2(RNA_seq_v2_mat + 1),
                  new.sort[seq(10)],splits = 8)

TCGA.MB1.CV.RNAseq.df <- data.frame(Genes = rownames(RNA_seq_v2_mat),
                                CV = as.numeric(TCGA.MB1.cv))


TCGA.MB1.RNAseq.df <- new.MB1.sort.df 

save(list = c('TCGA.MB1.RNAseq.df','TCGA.MB1.CV.RNAseq.df'),
      file = 'TCGA_MB1_RNASeq_data_nonorm.RData')


# MB1 lookup ----
ggplot(new.MB1.sort.df , aes(MB1.index, MB1.pc1)) +
